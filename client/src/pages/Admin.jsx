...
function Admin() {
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      await axios.put("https://server-production-0086.up.railway.app/settings", payload, {
  ...
  const handleAddQuestion = async () => {
    try {
      const token = localStorage.getItem("token");
      await axios.post("https://server-production-0086.up.railway.app/add-question", form, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`https://server-production-0086.up.railway.app/delete-question/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
